...
- `4c01/4d01` (ctor/dtor list 追記) は parser で受理できる。
- writer 側の副作用（`CTOR_LIST_PTR` / `DTOR_LIST_PTR` への書き込み）は未移植。
  - 現在は誤リンク防止のため、`4c01/4d01` を検出した時点で明示エラーにしている。
- `.doctor/.dodtor` (`e00c/e00d`) も現在は副作用未移植のため明示エラー。
- `c00c/c00d`（ctor/dtor section header）も未移植のため明示エラー。
...
### 3. CLI互換 (`main.s`) の未移植
- 現在のCLIは `-o/-r/--rn/--makemcs/--omit-bss/-p/-v/-x` 中心。
