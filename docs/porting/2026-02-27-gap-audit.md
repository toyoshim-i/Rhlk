...
> 最新状態は [2026-02-27-remaining-roadmap.md] と [2026-02-27-final-executable-compare.md] を正とする。
...
### 3. CLI互換 (`main.s`) の未移植
- 現在のCLIは `-o/-a/-an/-b/-d/-e/-g/-i/-l/-L/-r/--rn/--makemcs/--omit-bss/-p/-s/-t/-v/-z/-w/-x/-0/-1/--version` 中心。
...
- `-l<lib>` は `lib<lib>.a` を解決して入力へ追加、`-L` は検索パス追加、`-l` 単独は `LIB` 環境変数検索を有効化。
- `-i file` は現状「ファイル内トークンを入力ファイル列へ追加」互換で実装（オプション再帰展開は未実装）。
- `-0/-1` は ctor/dtor 互換モード切り替えとして実装（`-0` 時は ctor/dtor 関連命令をエラー化）。
...
## 2026-02-27 追記（map ヘッダパス方針確定）
- map ヘッダ2行目は実行時パスをそのまま出力する方針に固定。
- 回帰比較はヘッダ2行目のみ `<PATH>` 正規化し、それ以外は raw 一致を要求。
- 方針詳細は [docs/porting/2026-02-27-map-path-policy.md](2026-02-27-map-path-policy.md) に分離した。
